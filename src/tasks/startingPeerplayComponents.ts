...
import {indexing_all_databases} from "../databases/indexes/toolkit";
...
export const startingPeerplayComponents = new Listr([
  ...
  },
  {
    ...
    task: async (ctx,task) => {
      ...
      const PeerplayData: PeerplayData = ctx.peerplay_data
      monitor.start(PeerplayData.PublicAPIPort, PeerplayData.PrivateAPIPort, PeerplayData.JWT_SECRET)
      task.title = chalk.green(task.title + " [OK]")
  ...
  {
    title: 'Starting Database Syncronisation Process',
    task: async (ctx, task) => {
      const PeerplayData: PeerplayData = ctx.peerplay_data
      if (syncronize_all_databases(PeerplayData.DatabaseSyncLimit,PeerplayData.CustomDatabaseList) === "SUCCESS" && await indexing_all_databases() === "SUCCESS"){
        task.title = chalk.green(task.title + " [OK]")
        task.output = `Database Syncronisation Started Successfuly` + " [OK]";
      }
      else
