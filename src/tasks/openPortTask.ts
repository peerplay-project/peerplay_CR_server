...
export const openPortTask = new Listr(
  [
    ...
    },
    {
      ...
      task: async (ctx, task) => {
        const PeerplayData: PeerplayData = ctx.peerplay_data;
        UpnpKeeper(PeerplayData, upnpConfig);
...
const mappingTCPOrUDP = (results: any[], Port: number, tcpOrUdp: string) => {
  return results.find(
    (result: any) =>
      ...
      result.enabled === true &&
      ipAddressChecker(result.private.host) === true
  );
};
  ...
  try {
    ...
    return {
      ...
      upnp_config_results: {
        ...
        output: `Port successfully opened : ${openPortResults.title}`,
...
const UpnpKeeper = async (
  ...
  const upnpConfigList = {
    ...
    tcpOrUdp: ["TCP", "UDP", "TCP", "UDP", "TCP", "UDP"],
  };
  const stage = [0, 1, 2, 3, 4, 5];
  while (true) {
    try {
      for (const number of stage) {
        if (upnpConfigList.check[number] === true) {
