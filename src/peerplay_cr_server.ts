...
async function main(argv: string[]) {
  ...
  const JWT_SECRET = argsObj.jwt_secret || uuidv4();
      ...
      .catch((err: Error) => {
        ...
        switch (err.message) {
          ...
          case chalk.red.underline("Verify Database Port [FATAL_ERROR]"):
            ...
            break;
          case chalk.red.underline("Verify API Port [FATAL_ERROR]"):
            error_causes = Verify_Port_Failed(
              tasks.ctx,
              "API",
              tasks.ctx.peerplay_data.PublicAPIPort
            );
            break;
        }
...
const Obtain_Public_IP_Failed = function (ctx: Ctx) {
  let errors_cause: error_pack[] = [], error_description = "", error_solutions: string[] = [],
    error_name_ipv6 = "UNKNOWN", error_name_ipv4 = "UNKNOWN";
...
const Verify_Port_Failed = function (ctx: Ctx, codename: string, port: number) {
  ...
  if (error_name !== "UNKNOWN") {
    const public_port = port + ctx.peerplay_data.MinimalPortRange;
    ...
    const error_pack: error_pack = {
      code: "UPNP/" + codename + " : Failed to open port '" + port + "' with UPNP [ERROR]",
      description: error_description,
      solutions: error_solutions,
    };
    errors_cause.push(error_pack);
